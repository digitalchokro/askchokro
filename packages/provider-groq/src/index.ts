/**
 * @digitalchokro/provider-groq — Groq AI Provider
 *
...
export class GroqProvider implements AIProvider {
  ...
  constructor(config: GroqProviderConfig = {}) {
    this.config = config;
    const keyString = config.apiKey || process.env.GROQ_API_KEY || '';
  ...
  private rotateKey(): boolean {
    if (this.currentClientIndex < this.clients.length - 1) {
      ...
      console.warn(`[GroqProvider] Rate limit hit. Rotating to API key index ${this.currentClientIndex}.`);
      return true;
    }
    return false;
  }
